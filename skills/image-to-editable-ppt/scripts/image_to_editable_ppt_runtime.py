...
def configured_python():
    explicit = os.getenv("IMAGE_TO_EDITABLE_PPT_PYTHON")
    if explicit:
        return Path(explicit).expanduser(), "IMAGE_TO_EDITABLE_PPT_PYTHON"
    if os.name == "nt":
        conda_python = Path(r"C:\ProgramData\Anaconda3\envs\image-to-editable-ppt\python.exe")
        if conda_python.exists():
            return conda_python, "conda:image-to-editable-ppt"
    return None, None


def local_venv_python(home=None):
...
def venv_python(home=None):
    python, _source = configured_python()
    return python or local_venv_python(home)
...
def bootstrap(_args):
    home = runtime_home()
    env_dir = home / ".venv"
    python, source = configured_python()
    if python:
        if not python.exists():
            raise SystemExit(f"configured runtime Python is missing: {python}")
        print(f"Using configured runtime Python ({source}): {python}")
    else:
        python = local_venv_python(home)
    if not source and not python.exists():
...
def doctor(_args):
    ...
    _configured, source = configured_python()
    ok = True
    print(f"skill root: {skill_root()}")
    print(f"runtime home: {home}")
    source_label = source or "local .venv"
    print(f"runtime python: {python} ({'exists' if python.exists() else 'missing'}; {source_label})")
    if not python.exists():
        ok = False
    ...
    if not magick:
        ...
        if convert and not (os.name == "nt" and "system32" in str(Path(convert).parent).lower()):
            magick = convert
