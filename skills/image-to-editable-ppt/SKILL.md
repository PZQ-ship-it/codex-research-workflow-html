...
本仓库已创建专用 conda 环境 `image-to-editable-ppt`。运行本 skill 的 Python 脚本时优先使用：

```powershell
C:\ProgramData\Anaconda3\envs\image-to-editable-ppt\python.exe
```

不要使用当前默认 base Python 3.7 运行本 skill；`PyMuPDF>=1.24.0` 和 `Pillow>=10.0.0` 已安装在上述 conda 环境中。如环境丢失，重新创建：

```powershell
conda create -y -n image-to-editable-ppt python=3.11 pip
C:\ProgramData\Anaconda3\envs\image-to-editable-ppt\python.exe -m pip install -r .\skills\image-to-editable-ppt\requirements.txt
```
