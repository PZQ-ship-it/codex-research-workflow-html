...
- 缺少 `OPENROUTER_ICU_API_KEY` 时先询问用户提供 key，不发起请求。CLI 会先读取环境变量，也会从 skill 目录下的私密 `.env` 读取 `OPENROUTER_ICU_API_KEY=...`；不要把真实 key 写入仓库文件、文档或聊天记录。
...
从 skill 目录运行脚本，或按实际安装位置传入脚本路径。使用 Python 3 启动器：macOS/Linux 通常用 `python3 scripts/openrouter_icu_image.py`；Windows PowerShell/CMD 可用指向 Python 3 的 `python scripts\openrouter_icu_image.py`，如果安装了 Python launcher 也可用 `py -3 scripts\openrouter_icu_image.py`。脚本只使用 Python 标准库，不依赖 Bash、curl、chmod 或 POSIX 文件权限。
