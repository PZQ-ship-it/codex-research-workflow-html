...
def load_local_env() -> None:
    """Load OPENROUTER_ICU_API_KEY from a private .env next to the skill."""

    env_path = Path(__file__).resolve().parents[1] / LOCAL_ENV_FILE
    ...
    try:
        ...
        raise OpenRouterImageError(f"cannot read local env file: {env_path}") from exc
    ...
    for line in lines:
        ...
        if len(value) >= 2 and value[0] == value[-1] and value[0] in {"'", '"'}:
            value = value[1:-1]
        if key == "OPENROUTER_ICU_API_KEY" and value and key not in os.environ:
